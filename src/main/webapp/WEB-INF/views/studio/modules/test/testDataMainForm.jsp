...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/test/testDataMain/">主表列表</a></li>
		<li class="active"><a href="${ctx}/test/testDataMain/form?id=${testDataMain.id}">主表<shiro:hasPermission name="test:testDataMain:edit">${not empty testDataMain.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="test:testDataMain:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
				...
				<div class="controls">
					...
					<script type="text/template" id="testDataChildTpl">//<!--
						<tr id="testDataChildList{{idx}}">
							...
							<td>
								<input id="testDataChildList{{idx}}_name" name="testDataChildList[{{idx}}].name" type="text" value="{{row.name}}" maxlength="100" class="input-small required"/>
							</td>
							<td>
								<textinput id="testDataChildList{{idx}}_remarks" name="testDataChildList[{{idx}}].remarks" rows="4" maxlength="255" class="input-small ">{{row.remarks}}</textarea>
							</td>
